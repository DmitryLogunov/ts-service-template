...
// WARNING! Coused by the feature of SQLITE functionality with executing sql queries with REGEXP
//          this test only partly checks 'regexp' operand (see fake-db-client.ts:123 ) !!!

test(`GET /${resource}?filter[info][regexp]=ww`, async () => {
  const filtersParams = '?filter[info][regexp]=ww';
